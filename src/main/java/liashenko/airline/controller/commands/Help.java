...
public abstract class Help {
    ...
    private final static String SHOW_AVAILABLE_AIRCRAFT_MODELS_COM = String.format("type \"%s\" to get list of " +
            "available aircraft models", CommandsHelper.SHOW_MODELS_COMMAND);

    private final static String ADD_COM = String.format("type \"%s %s aircraft_producer_name %s aircraft_model_name\" " +
                    "to add it to the airline park", CommandsHelper.ADD_COMMAND, CommandsHelper.PRODUCER_NAME_COMMAND_KEY,
            CommandsHelper.MODEL_NAME_COMMAND_KEY);

    private final static String REMOVE_COM = String.format("type \"%s %s aircraft_serial_number_id\" to remove it " +
    ...
    private final static String FIND_AIRCRAFT_BY_FUEL_CONSUMT_PARAM_COM = String.format("type \"%s  %s" +
                    "fuel_consumption_degree\" to find the aircraft in the airline park by fuel consumption degree",
    ...
    private final static String CALCULATE_TOTAL_AND_CARRYING_CAPACITY = String.format("type \"%s\" to calculate " +
            "total and carrying capacity of the aircraft from the airline park", CommandsHelper.CALCULATE_CAPACITY_COMMAND);

    private final static String SAVE = String.format("type \"%s\" info on disk", CommandsHelper.SAVE_COMMAND);
